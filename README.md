...
- Paper waivers: photograph one on the admin page, check the details Claude
  reads off it, and it is stored, emailed and sent to MatTracker like an online
  waiver, with the photo cropped to the page as its signature
...
- `POST /api/admin/paper-waivers/read` - read a photo of a paper waiver with
  Claude: its fields and the page's corners (nothing is stored)
- `POST /api/admin/paper-waivers` - store a checked paper waiver with its photo
...
waiver. The photo goes to Claude (`claude-opus-5`), which reads the handwriting
into the same fields as the online form and finds the corners of the page. The
admin drags the corners if needed, corrects the details, confirms the page is
signed, and saves. The browser then flattens the page out of the photo -
nothing outside the paper is kept - and that image is stored as the waiver's
signature. From there it is the same as an online waiver: one row per person,
the PDF (with the photo on its own page) emailed to the gym and the signer, the
MatTracker account, and the follow-up a week after the date written on it.
...
Reading needs `ANTHROPIC_API_KEY` in the backend's environment (on the cluster,
the `ANTHROPIC_API_KEY` key of `app-secrets`). Without it the photo is still
cropped and stored, and the details are typed in by hand.
